...
test:
	@echo "  > Test Iteration ..."
	cd bin && ./gophermarttest -test.v -test.run=^TestGophermart$$ -gophermart-binary-path=cmd/gophermart/gophermart -gophermart-host=localhost -gophermart-port=$(GOPHER_MART_PORT) -gophermart-database-uri="postgresql://postgres:postgres@postgres/praktikum?sslmode=disable" -accrual-binary-path=cmd/accrual/accrual -accrual-host=localhost -accrual-port=$(ACCRUAL_PORT) -accrual-database-uri="postgresql://postgres:postgres@postgres/praktikum?sslmode=disable"
