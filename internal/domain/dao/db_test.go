/*
 * This file was last modified at 2024-04-15 21:31 by Victor N. Skurikhin.
 * db_test.go
 * $Id$
 */
...
func TestDaoSaveGet(t *testing.T) {
	...
	var tests = []struct {
	...
	}{
		{
			name: "Test #1 User",
			save: func() (interface{}, error) {
				store = store.WithContext(utils.NewUUIDContext())
				repo0 := Users(store)
...
func TestStatus(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			repo2 := Statuses(store.WithContext(utils.NewUUIDContext()))
